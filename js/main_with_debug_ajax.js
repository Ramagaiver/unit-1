...
function debugCallback(megaCitiesData){
	document.querySelector("#geoJson").insertAdjacentHTML('beforeend', 'GeoJSON data: ' + JSON.stringify(megaCitiesData));
};

// Fetches the MegaCities.geojson
function debugAjax(){	
	fetch('data/MegaCities.geojson')
		.then(function(megaCitiesData){
			return megaCitiesData.json();
		})
		.then(function(megaCitiesData){
			console.log(megaCitiesData)
			debugCallback(megaCitiesData)
		})
};

// Calls citiesTable & debugAjax
initialize();
